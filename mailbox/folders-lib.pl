...
# 2007/02/19 kabe:
#  also skip Maildir/courierpop3dsizelist file from recurse list
...
if ($f->{'sortable'}) {
	...
	# For each mail in the index, 
	local @sorted = sort { $b->{'idx'} <=> $a->{'idx'} } @_;
	foreach my $k (sort { $a <=> $b } (keys %index)) {
		local ($ki, $kf) = split(/_/, $k, 2);
		# Skip indexes lower than the first being deleted
		next if ($ki < $sorted[$#sorted]->{'idx'});	
		foreach my $m (@sorted) {
			if ($ki == $m->{'idx'}) {
				# This is an index entry for a deleted email
				delete($index{$k});
				last;
				}
			elsif ($ki > $m->{'idx'}) {

...
if ($remote_user && @_ > 1) {
	...
	if (!$ENV{'LOCAL_USER'}) {
		print qq~<td width="84" nowrap><a href='$gconfig{'webprefix'}$logout'><img src="$gconfig{'webprefix'}/images/top_bar/$loicon" width=$lowidth height="31" border="0" alt="$lotext"></td>~;
		}
...
if (@_ > 1 && !$one && $remote_user && !$gconfig{'notabs'}) {
   ...
   if ($ENV{'HTTP_WEBMIN_SERVERS'}) {
	...
	if ($_[3]) {
		...
		if (!$access{'noconfig'} && !$config{'noprefs'}) {
			local $cprog = "uconfig.cgi";
			&tab_start; print "<a href=\"$gconfig{'webprefix'}/$cprog?$module_name\">",
...
print $_[1];
print "</head>\n";
print '<body bgcolor=#6696bc link=#000000 vlink=#000000 text=#000000 leftmargin="2" topmargin="2" marginwidth="2" marginheight="2" '.$_[2].'>';
}

...
&scmd(1);
$proxy = join(" ", &find_value("ProxyCommand", $conf));
print "<td><b>$text{'host_proxy'}</b></td> <td nowrap colspan=3>\n";
printf "<input type=radio name=proxy_def value=1 %s> %s\n",
	$proxy ? "" : "checked", $text{'host_proxy_def'};
printf "<input type=radio name=proxy_def value=0 %s>\n",
	$proxy ? "checked" : "";
print "<input name=proxy size=60 value='$proxy'></td>\n";
&ecmd();

if (&get_ssh_type() eq 'openssh') {
	&scmd();
